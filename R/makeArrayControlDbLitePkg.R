#' create a ArrayControlDbLite package from sqlite file (usually via repDbLiteFromFasta)
...
#' 
#' @export
makeArrayControlLitePkg <- function(erccdblitefile=NULL, author="Nobody", email="dev@null.com", version="1.0", destDir=".", ...) {
  
  stopifnot(class(erccdblitefile) == "character")
  erccdb <- ArrayControlDbLite(x=erccdblitefile)
  md <- metadata(erccdb)
  fetchMeta <- function(x) md[x, "value"]
  pkg <- fetchMeta("package_name")

  maintainer <- paste0(author, " <", email, ">")
  template_path <- system.file("arraycontroldblite", package="TxDbLite")
  release_date <- fetchMeta("creation_time")
  symvals <- list(
    PKGTITLE="ArrayControl annotation package",
    PKGDESCRIPTION="lightweight ArrayControl spike-in annotations",
    PKGVERSION=version,
    AUTHOR=author,
    ...
    ORGANISM="N/A", 
    SPECIES="N/A", 
    PROVIDER="ArrayControl/NIST",
    PROVIDERVERSION="N/A",
    RELEASEDATE=release_date,
    SOURCEURL="http://www.nist.gov/mml/bbd/ercc.cfm",
    ORGANISMBIOCVIEW="ArrayControl",
    TXDBOBJNAME=pkg
  )
