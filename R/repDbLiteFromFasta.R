...
repDbLiteFromFasta <- function(fastaFile, verbose=TRUE) {
  ...
  hit <- !is.na(match(tolower(uncataloged), tolower(rownames(repeat_biotypes))))
  ...
  message(length(uncataloged), " uncataloged repeat biotypes, fix Tiggers...")
  Tigger <- repeat_biotypes["Tigger1", , drop=FALSE]
  Tiggers <- uncataloged[which(toupper(substr(uncataloged, 1, 6)) == "TIGGER")]
  ...
  HERVi <- paste0(HERVs[paste0(HERVs, "-int") %in% rownames(repeat_biotypes)], 
                  "-int")
  ...
  message(length(uncataloged), " uncataloged repeat biotypes, fix Mariners...")
  ...
  hinted$gene_biotype[grep("Transposable", hinted$tx_biotype)] <- "DNA_element"
  hinted$gene_biotype[grep("EUT",ignore=T, hinted$tx_biotype)] <- "DNA_element"
  hinted$gene_biotype[grep("SAT",ignore=T, hinted$tx_biotype)] <- "other_repeat"
  ...
  hinted$gene_biotype[grep("Repetitive", hinted$tx_biotype)] <- "other_repeat"
  hinted$gene_biotype[grep("Endogenous", hinted$tx_biotype)] <- "LTR_element"
  ...
  repeat_biotypes <- repeat_biotypes[order(repeat_biotypes$name),] 
  uncataloged <- setdiff(names(txs), rownames(repeat_biotypes))
  message(length(uncataloged), " uncataloged repeat biotypes after hinting.")
